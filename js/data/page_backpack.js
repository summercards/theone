...
function onTouchstart(e) {
  ...
  const x = t.clientX, y = t.clientY;
    if (x >= backBtnArea.x && x <= backBtnArea.x + backBtnArea.width &&
      ...
      // 点击背包的“返回”按钮。
      // 优先从全局变量获取预设的返回页面，再回退使用持久化存储；最后默认回首页。
      let returnPage = null;
      if (globalThis._backpackReturnPage) {
        returnPage = globalThis._backpackReturnPage;
      } else if (typeof wx !== 'undefined' && wx.getStorageSync) {
        try { returnPage = wx.getStorageSync('_backpackReturnPage'); } catch (_e) { returnPage = null; }
      }
      const targetPage = returnPage || 'home';
      // 清理返回页标记（全局和本地存储）
      if (typeof wx !== 'undefined' && wx.removeStorageSync) {
        try { wx.removeStorageSync('_backpackReturnPage'); } catch (_e) {}
      }
      if (globalThis._backpackReturnPage) {
        delete globalThis._backpackReturnPage;
      }
      // 跳转回原页面
      if (typeof switchPageFn === 'function') {
        switchPageFn(targetPage);
      }
    }
}
